...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'flexity' ),
			'menu_label'            => __( 'Vafpress Options', 'flexity' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(__( 'Option Key is required', 'flexity' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'flexity' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'flexity' ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'flexity'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'flexity');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'flexity');
				}
			}
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'flexity');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access", 'flexity');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'flexity' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'flexity'));
			...
			$restore_section->set_title(__('Restore Default', 'flexity'));
			...
			$exim_section->set_title(__('Export/Import', 'flexity'));
